#pragma once

static const auto PHOENIX_VACCOUNT = "phoenix"_n;
static const auto PHOENIX_FEES_VACCOUNT = "fees.phoenix"_n;
static const name token_account = name("phoenixtokn3");
static const name phoenix_account = name("phoenixashe5");

static const extended_symbol WEOSDT_EXT_SYMBOL{symbol("AIRZ", 4),
                                               name("weosdttokens")};
static const double FEES_PERCENTAGE = 0.02;
static const name FEES_ACCOUNT = name("phoenixashes");