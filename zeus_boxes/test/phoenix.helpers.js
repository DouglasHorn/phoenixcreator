...
const { encodeName } = require("../dapp-services/services/dapp-services-node/common");

const url = getUrl(getDefaultArgs());
const rpc = new JsonRpc(url, { fetch });

const addToAuth = async (code, providers = []) => {
  const dspeos = await getLocalDSPEos(code);
  const eosioContract = await dspeos.contract(`eosio`);
  var keys = await getCreateKeys(code);
  console.log(Object.keys(dspeos));
  console.log(Object.keys(eosioContract));
  console.log(keys);
  ...
  console.log(
    JSON.stringify((await dspeos.rpc.get_account(code)).permissions, null, 2)
  );
};
...
const initHelpers = ({ endpoint }) => {
  ...
  const runTrx = async ({ nonce = 0, contract_code, payload, wif }) => {
    ...
    console.log(`vaccount transaction sent`, trxResult.result)
